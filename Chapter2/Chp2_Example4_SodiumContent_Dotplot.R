...
# Create Dotplot:
stripchart(Sodium, method="stack", pch=19, ylim=c(0,3), frame.plot=FALSE, xlab="Sodium Content (mg)", main="Dotplot")
...
ggplot(data.frame(Sodium), aes(x=Sodium)) + 
  geom_dotplot() +
  labs(x="Sodium Content (mg)", title="Dotplot", subtitle="Sodium Content of 20 Breakfast Cereals") +
  theme_classic() +
